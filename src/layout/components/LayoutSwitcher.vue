<script setup lang="ts">
  ...
  const layouts: { value: LayoutMode; label: string }[] = [
    { value: 'sidebar', label: '经典侧边栏' },
    { value: 'top', label: '顶部导航' },
    { value: 'mixed', label: '混合导航' },
    { value: 'dual', label: '双列侧边栏' }
  ]
...
<template>
  ...
  >
    <button
      ...
      @click="open = !open"
    >
      <DAliIcon name="view-quilt-outline" :size="18" />
    ...
    <div class="layout-switcher__panel">
      ...
      >
        ...
        <DAliIcon v-if="appStore.layout === item.value" name="check" :size="16" />
      </button>
    </div>
  </div>
</template>
