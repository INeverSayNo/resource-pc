...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    function handleEdit(row: any) {
      currentRow.value = deepClone<any>(row);
      nextTick(() => {
        showEdit.value = true;
      });
    }
    function handleAddRow() {
      currentRow.value = deepClone<any>({}); 
      nextTick(() => {
        showEdit.value = true;
      });
    }
    ...
    //     Message.warning("请选中需要删除的类型信息！");
    //     return;
    //   } */
    //   ElMessageBox.confirm(
    //     "是否删除选中行的类型信息，删除后不能恢复？",
    //     "提示信息",
    //     { type: "warning" }
    //   ).then(() => {
    ...
    // function handleChangeRow(row: any) {
    //   if (row) {
    ...
    // function GetData() {
    //   const temp = deepClone<any>(data.value);
    //   return temp.filter((x) => x.type);
    // }
    const loading = ref(false);
    const datalis = ref<any>();
    function dataLoad() {
      loading.value = true
      api.OpionDefine(`trafficability-queryById?id=` + props?.stationId, null, "GET")
        .then((res) => {
          console.log(res)
