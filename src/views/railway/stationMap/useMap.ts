...
export function useMap(filterState: FilterState) {
  ...
  // 当前用户详细位置
  const currentLocation = ref<Record<'lat' | 'lng', string | number> & { province: string }>()
    ...
    MapInstance.value.onzoomend = () => {
      ...
      if (zoomLevel <= 6) {
        ...
        if (labelOverlayIndex > -1) {
          MapInstance.value?.removeOverlay(addressMarkerList.value[labelOverlayIndex])
  ...
  function initMarkerIcon() {
    markerIconState.normalIcon = new BMapGL.Icon(normalIcon, new BMapGL.Size(30, 30))
    markerIconState.privateLineBmpIcon = new BMapGL.Icon(privateLineIcon, new BMapGL.Size(18, 18))
    markerIconState.portBmpIcon = new BMapGL.Icon(portIcon, new BMapGL.Size(25, 25))
    markerIconState.containerBmpIcon = new BMapGL.Icon(containerIcon, new BMapGL.Size(30, 30))
    markerIconState.addressIcon = new BMapGL.Icon(addressLocation, new BMapGL.Size(30, 30))
  }
  ...
  function renderStation(isClearOverlays = true, isKeepNavgate = false) {
    ...
    if (bounds) {
      ...
      if (['city', 'province'].some((e) => e === optionsState.levelMark)) {
        if (optionsState.levelMark === 'province') {
          ...
          viewAreaProvinceList.forEach((province) => {
            province.cityList.forEach((city) => {
              ...
              if (city.cityLocation && city.cityLocation.lat !== 0 && city.cityLocation.lng !== 0) {
                addProvinceCityMarkerLabel(
                  city.cityName,
                  city.cityLocation!,
        ...
        const viewAreaStationList = stationPortList.value.reduce((prev, next) => {
          next.cityList.forEach((city) => {
            prev.push(
              ...city.stationList.filter((e) => bounds.containsPoint(e.station.formatLocation))
            )
            viewAreaPortList.push(
              ...city.portList.filter((e) => bounds.containsPoint(e.station.formatLocation))
            )
          })
          return prev
  ...
  function addMarkerLable(
    ...
    for (const stationPortItem of payload) {
      const isStation = Reflect.has(stationPortItem.station, 'railwayStationName')
      const marker = new BMapGL.Marker(stationPortItem.station.formatLocation, {
        icon: isStation ? markerIconState.normalIcon! : markerIconState.portBmpIcon!
      })
      if (isStation && checkStationHasContainer(stationPortItem as Dashboard.StationItem)) {
        marker.setIcon(markerIconState.containerBmpIcon!)
      }

      const label = createLabel(stationPortItem.station.formatLocation, stationPortItem)
      MapInstance.value?.addOverlay(label)
      MapInstance.value?.addOverlay(marker)
      const markerAndLabelEvent = () => {
        oldPrivateLineMarker.value.forEach((y) => MapInstance.value?.removeOverlay(y))
        ...
        MapInstance.value?.openInfoWindow(infoWindow, stationPortItem.station.formatLocation)
      }
    ...
    Promise.all([GetAllStation(), GetAllPort()]).then((res) => {
      ...
      if (Array.isArray(stationListRsp) && stationListRsp.length) {
        const result = stationListRsp
          .filter((e) => Boolean(e.station.provinceName) && Boolean(e.station.railwayLocation))
          .reduce((prev, next) => {
            const [railwayLng, railwayLat] = next.station.railwayLocation.split(',')
            const [bdLng, bdLat] = wgs84tobd09(+railwayLng, +railwayLat)
            next.station['formatLocation'] = new BMapGL.Point(bdLng, bdLat)
            if (prev.every((item) => item.provinceName !== next.station.provinceName)) {
              const location = next.station.provinceLocation?.split(',')
              const point = location ? new BMapGL.Point(+location[0], +location[1]) : null
              prev.push({
                ...
                cityList: [
                  {
                    ...
                    cityLocation: getCityLocation(next.station.cityLocation, next.station.address),
                    portList: [],
                    stationList: [next]
                  }
                ]
              ...
              if (prev[index].cityList.every((item) => item.cityName !== next.station.cityName)) {
        ...
        portListRsp.forEach((portItem) => {
          const [waterwayLat, waterwayLng] = portItem.station.portCoord.split('/')
          const [bdLng, bdLat] = wgs84tobd09(+waterwayLng, +waterwayLat)
          portItem.station['formatLocation'] = new BMapGL.Point(bdLng, bdLat)
          const provinceItem = result.find((e) => e.provinceName === portItem.station.provinceName)
          if (provinceItem) {
  ...
  function renderRouteQueryStation() {
    if (name) {
      let matchStationPort: Dashboard.PortItem | Dashboard.StationItem | null = null
      for (let i = 0; i < stationPortList.value.length; i++) {
        for (let j = 0; j < stationPortList.value[i].cityList.length; j++) {
          const matchStation = stationPortList.value[i].cityList[j].stationList.find(
            (e) => e.station.railwayStationName === name
          )
          if (matchStation) {
    ...
    if (!provinceName && !Reflect.has(payload, 'provinceName')) {
      ...
      geoc.getLocation(point, (rs) => {
        ...
        SearchNearStation(param)
          .then((res) => {
            nearState.nearList = res.data.sort((x, y) => (x.distance > y.distance ? 1 : -1))
          })
          .finally(() => {
  ...
  function handleSelectNearStationPort(
    ...
    if (matchStationPort) {
      calcuteNavigationDistance(matchStationPort).then((navgationDistance) => {
  ...
  function calcuteNavigationDistance(current: Dashboard.PortItem | Dashboard.StationItem) {
    return new Promise<number>((resolve) => {
      try {
        ...
        const arrivep = new BMapGL.Point(
          searchNearStationParams.value?.longitude,
          searchNearStationParams.value?.latitude
        )
        var transit = new BMapGL.DrivingRoute(MapInstance.value!, {
          ...
          onSearchComplete: (results) => {
            ...
            const distance = (plan.getDistance(false) as number) / 1000.0
            let destName = ''
          ...
          onMarkersSet: (points) => {
            navigationState.mapRouteOverlays.start =
              points.find((x) => x.title === '起点')?.marker || null
            navigationState.mapRouteOverlays.end =
              points.find((x) => x.title === '终点')?.marker || null
          }
        })
  ...
  function loadTempStationPort(cityName: string) {
    storage.getItem('stationPortList').then((res) => {
      if (!res) {
        Promise.all([GetStationByCityName(cityName), GetPortByCityName(cityName)]).then((res) => {
          if (res) {
  ...
  function addAddressMarker(point: BMapGL.Point, payload: ChooseItem, routerAction: Router) {
    ...
    sendBtn.innerHTML += "<span class='dashboard-send-there-label'>从这发货</span>"
    const arriveBtn = document.createElement('p')
    render(h(DAliIcon, { name: 'arrive' }), arriveBtn)
    arriveBtn.innerHTML += "<span class='dashboard-arrive-there-label'>到货这里</span>"
    ...
    ;(titleEl.firstElementChild as HTMLSpanElement).style.textOverflow = 'ellipsis'
  ...
  onMounted(() => {
    ...
    getAK().then((ak) => {
      loadJs('//api.map.baidu.com/api?v=1.0&type=webgl&callback=initialize', ak, () => {
        initMap()
        calcHeight()
      })
    })
...
function getCityLocation(cityLocationStr: string | null, stationLocation: string | null) {
  if (cityLocationStr) {
...
function createLabel(
  ...
  if (Reflect.has(data, 'station')) {
    style.color = renderLevelColor((data as Dashboard.StationItem).station.stationLevelMark || '')
  }
  label.setStyle(style as any)
  return label
}
...
function getNameLength(name: string) {
  ...
  return (name.length - anumLength - startLength) / 2 + anumLength / 4 + startLength / 3.3
}
...
function createInfoWindow(
  ...
  })</b><span class='theme-danger mr-20px fr'>${data.station.rate.toFixed(2)}%</span>`
  const opts = {
    title: title, // 信息窗口标题
  ...
  near.addEventListener('click', () => {
    ...
    loadNearStation(p, data.station.provinceName, data.station.id, data.station.railwayStationName)
  ...
  sendThere.innerHTML += "<span class='dashboard-send-there-label'>从这发货</span>"
  ...
  arriveThere.innerHTML += "<span class='dashboard-arrive-there-label'>到货这里</span>"
...
function createPortInfoWindow(
  ...
  near.addEventListener('click', () => {
    ...
    loadNearPort(p, data.station.provinceName, data.station._id, data.station.portAreaName)
  ...
  function toQuerySolution(type: 'start' | 'arrive') {
    const [province = '', city = '', district = ''] = (data.station?.portAddress || '').split(',')
    sessionStorage.setItem(
      'tgsSolutionQueryParams',
      JSON.stringify({
...
function createStationEl(station: Dashboard.RailwayStationDto) {
  ...
  renderStationProperties(station).forEach((x) => {
    propertieStr += `<span class='propertie-item ${x.class || ''}'>${x.value}</span>`
  })
  ...
  div.appendChild(createContentEl('办理范围', station.scopeOfBusiness, 'theme-success'))
  ...
  const phone = station.principalPhone ? document.createElement('span') : undefined
  if (phone) {
...
function createStationNoticeEl(station: Dashboard.RailwayStationDto, routerAction: Router) {
...
function createContentEl(label: string, val: any, css?: string, el?: HTMLElement) {
  const div = document.createElement('div')
...
function createPrivateLineMark(
  ...
  return (
    zyxList?.map((x, index) => {
      ...
      const marker = new BMapGL.Marker(point, {
        icon: hasContainer ? markerIconState.containerBmpIcon! : markerIconState.privateLineBmpIcon!
      })
...
/** 创建专用线信息窗 */
function createPrivateLineInfoWindow(data: Dashboard.StationItem['zyxList'][number]) {
  const title = `专用线信息`
  const opts = {
    width: 250, // 信息窗口宽度
    title: title, // 信息窗口标题
  ...
  text += `<p><span class=\'map-window-label\'>联系方式：</span>${
    data.contacts || ''
  }${data.phone || ''}</p>`
...
function getCirclePoint(point: BMapGL.Point, deg: number, circleR: number) {
  ...
  const diffy = (Math.round(Math.sin((deg * Math.PI) / 180) * 100000000000) / 100000000000) * r
  const diffx = (Math.round(Math.cos((deg * Math.PI) / 180) * 100000000000) / 100000000000) * r
...
function createPortEl(
  ...
  const tags = document.createElement('div')
  const tag = document.createElement('span')
  tag.setAttribute('class', `station-tag ${tagColorPortMap.get(station.portType)}`)
  tag.innerText = station.portType
  tags.appendChild(tag)
  div.appendChild(tags)
  const phone = station.areaUserTel ? document.createElement('span') : undefined
  if (phone) {
  ...
  div.appendChild(createContentEl('备注', station.portSpecification, 'theme-success'))
  div.appendChild(
    createContentEl(
      '区域/负责人',
  ...
  if (contact.length > 0) {
    const phones =
      contact !== null && contact !== undefined ? document.createElement('span') : undefined
    let users = ''
    if (phones) {
      ...
      div.appendChild(createContentEl('港口联系人', `${users || ''}`, '', phones))
    }
  } else {
