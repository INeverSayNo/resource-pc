<template>
  <DcGap class="bar">
    ...
    <span v-if="!isView" class="bar-btn fr fs-14 cu-pointer theme-danger" @click="handleDelRow">
      <DAliIcon name="delete" class="" />
      删除
    </span>
    <span
      v-if="!isView"
      class="bar-btn fr fs-14 mr-10px theme-color cu-pointer"
      @click="handleAddRow"
    >
  ...
  >
    ...
    <el-table-column label="新增人员" align="center" width="180" prop="validDate">
      <template #default="scoped">
        {{ scoped.row.creatorName }}
        {{ formatTime(scoped.row.creationTime, 'yyyy-MM-dd HH:mm') }}
      </template>
    </el-table-column>
    <el-table-column label="修改人员" align="center" width="180" prop="validDate">
      <template #default="scoped">
  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      function handleClickRow(row: RailwayStationaryUser) {
        ...
        if (editIndex.value !== -1 && !Reflect.get(data.value[editIndex.value], 'userId')) {
      ...
      function handleAddRow() {
        ...
        if (
          editIndex.value !== -1 &&
          data.value.filter((x) => data.value[editIndex.value].phone === x.phone)?.length > 1
        ) {
      ...
      function handleDelRow() {
        ...
        ElMessageBox.confirm('是否删除选中行的人员信息，删除后不能恢复？', '提示信息', {
          type: 'warning'
        }).then(() => {
