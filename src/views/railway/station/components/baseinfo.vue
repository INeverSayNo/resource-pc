    ...
    <el-row>
        ...
        <span v-else class="value">
          <template v-for="cItem in item.value as any[]" :key="cItem.value">
            <span
              ...
              class="theme-color mr-5px cu-pointer"
              v-html="createPrivatePhone(cItem.value).outerHTML"
            ></span>
            <span v-else class="mr-5px"> {{ cItem.value }}{{ cItem.suffix || '' }} </span>
          </template>
        </span>
      </el-col>
        ...
        <span class="value">
          <span v-for="(user, index) in stationaryUsers" :key="user.id">
            ...
            <span
              ...
              :class="{ 'mr-5px': index !== stationaryUsers.length - 1 }"
              v-html="createPrivatePhone(user.phone).outerHTML"
            ></span>
            <span v-if="index !== stationaryUsers.length - 1" class="mr-5px"> / </span>
          </span>
        </span>
      </el-col>
      <el-col :span="8">
      ...
      <el-col :span="24">
        ...
        <span class="value">
          ...
          <span class="mr-5px">
            {{ formatTime(station?.lastModificationTime || station?.creationTime, 'yyyy-MM-dd') }}
          </span>
        </span>
      </el-col>
    </el-row>
    ...
    setup(props, { emit }) {
      ...
      const getStationType = computed(
        () => (type: 'serviceFormat' | 'transactFormat' | 'natureTypesFormat') => {
          if (!Reflect.get(props.station || {}, type) || !Array.isArray(props.station?.[type])) {
            return '暂无数据'
          }
