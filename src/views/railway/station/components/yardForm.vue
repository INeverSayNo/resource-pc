<template>
  <com-dialog
    ...
    @close="visable = false"
  >
    <el-form v-if="edit" class="form-container" :model="edit" label-suffix=":" label-width="110px">
      <el-form-item label="办理范围">
        <el-input v-model="edit.handleScope.scope" placeholder="请输入办理范围"></el-input>
      </el-form-item>
      <el-form-item label="" label-width="110px">
        <el-checkbox-group v-model="scopeAttr">
          <el-checkbox v-for="item in scopeOption" :key="item.value" :label="item.value">
            {{ item.label }}
          </el-checkbox>
        </el-checkbox-group>
      </el-form-item>
      ...
      <DcGap class="mb-05">
        ...
        <span class="fr mr-10px theme-danger cu-pointer" @click="handleDeleteCharge">
      ...
      >
        ...
        <el-table-column label="办理类型" header-align="center" width="160" prop="chargeType">
        ...
        <el-table-column label="免费时间" width="250" header-align="center">
          <template #default="scoped">
            <el-input v-model="scoped.row.freeTime" placeholder="请输入免费时间">
              <template #append>
                <el-select
            ...
            >
              <template #append>
                <el-select
                  ...
                  placeholder="单位"
                >
                  <el-option label="元/吨.天" value="元/吨.天"> 元/吨.天 </el-option>
                  <el-option label="元/箱.天" value="元/箱.天"> 元/箱.天 </el-option>
                  <el-option label="元/车.天" value="元/车.天"> 元/车.天 </el-option>
                </el-select>
              </template>
            </el-input>
          </template>
      ...
      <DcGap class="mb-05">
        ...
        <span class="fr mr-10px theme-danger cu-pointer" @click="handleDelete">
          <DAliIcon name="delete" class="" />
          删除选中
        </span>
      </DcGap>
      ...
      >
        <el-table-column type="selection"></el-table-column>
        <el-table-column width="150" label="办理类型" header-align="center" prop="type">
          <template #default="scoped">
            <el-input v-model="scoped.row.type" placeholder="请输入办理类型"></el-input>
          </template>
        </el-table-column>
        <el-table-column label="发送货物" header-align="center" prop="fsInfo">
          <template #default="scoped">
            <el-input v-model="scoped.row.fsInfo" placeholder="请输入发送货物"></el-input>
          </template>
        </el-table-column>
        <el-table-column label="到达货物" header-align="center" prop="ddInfo">
          <template #default="scoped">
            <el-input v-model="scoped.row.ddInfo" placeholder="请输入到达货物"></el-input>
          </template>
        </el-table-column>
      </el-table>
    </el-form>
...
<script lang="ts" setup>
  ...
  import { DangerScope, RailWayGoodsYard, YardChargeItem, YardHandleScope } from '../types'
  ...
  function handleDelete() {
    ...
    ElMessageBox.confirm('是否删除选中行的信息，删除后不能恢复!', '提示信息').then(() => {
  ...
  function handleSave() {
    if (
      edit.value?.contributor === '' ||
      edit.value?.contributor === null ||
      edit.value?.contributor === undefined
    ) {
      Message.warning('请选择贡献人')
      return
    }
    if (
      edit.value?.contributionaTime === '' ||
      edit.value?.contributionaTime === null ||
      edit.value?.contributionaTime === undefined
    ) {
