<template>
  <DcGap class="bar with-input">
    ...
    <div class="fr">
      ...
      >
        <DLegacyIcon v-if="showType === 'details'" name="menu" class="" />
        <DLegacyIcon v-else name="s-operation" class="" />
      </span>
    </div>
  </DcGap>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    function handleEdit(row: RailWayPrivatelLine) {
      state.currentRow = deepClone<RailWayPrivatelLine>(row);
      state.showEdit = true;
    }
    ...
    function handleSearch() {
      const temp = deepClone<RailWayPrivatelLine[]>(props.list || []);
      selfList.value = temp
        .filter(
          (x) =>
    ...
    watch(
      () => props.list,
      (val) => {
        selfList.value = deepClone<RailWayPrivatelLine[]>(val || []).sort(
          (x, y) => {
            return x.lineType > y.lineType ? -1 : 1;
          }
        );
