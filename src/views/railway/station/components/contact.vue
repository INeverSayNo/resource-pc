<template>
  <DcGap class="bar">
    ...
    <span class="fs-12 theme-danger mr-5">
      (车站窗口、货运室等中铁路工联系方式)
    </span>
  ...
  <el-table :data="contacts" size="small" border stripe highlight-current-row :max-height="tableHeight">
    <el-table-column
      label="服务类别"
      align="center"
      width="110"
    ...
    <el-table-column label="联系人" align="center" width="300" prop="contacts">
      <template #default="scoped">
        ...
        ></span>
        <span v-if="scoped.row.phone && scoped.row.telePhone" class="mr-5 ml-5">
          /
        </span>
        <span
          class="theme-color cu-pointer"
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    function handleEdit(row: RailWayStationContacts) {
      currentRow.value = deepClone<RailWayStationContacts>(row);
      nextTick(() => {
        showEdit.value = true;
      });
    }
