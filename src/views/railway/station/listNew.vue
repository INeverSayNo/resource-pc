<template>
  ...
  >
    <template #queryBarCenter>
      <DcNotifyBar class="my-notify" :options="cmsShowList" @click="handleShowCms"></DcNotifyBar>
    </template>
    <template #queryform>
      <el-form-item label="车站名称">
        <el-input v-model="param.railwayStationName" placeholder="请输入车站名称"></el-input>
      </el-form-item>
      <el-form-item v-if="false" label="车站编码">
        <el-input v-model="param.railwayStationCode" placeholder="请输入车站编码,tmism"></el-input>
      </el-form-item>
      ...
      <el-form-item label="车务段">
        <el-select v-model="param.railwayTrainCode" placeholder="请选择车务段" filterable clearable>
          <el-option
      ...
      <el-form-item label="车站属性">
        <el-select v-model="param.stationProp" clearable placeholder="请选择车站属性">
      ...
      <el-form-item label="办理类型">
        <el-select v-model="param.transactType" clearable placeholder="请选择办理类型">
      ...
      <el-form-item label="城市">
        <el-select v-model="param.cityName" placeholder="请选择省份" filterable clearable>
          <el-option
      ...
      <el-form-item label="站点分级">
        <el-select v-model="param.railwayStationGrade" clearable placeholder="请选择是否优势站点">
          <el-option label="全部" value="_all"></el-option>
          <el-option
      ...
      <el-form-item label="专用线名称">
        <el-input v-model="param.privatelineName" placeholder="请输入专用线名称"></el-input>
      </el-form-item>
      <el-form-item label="驻站人员">
        <el-select v-model="param.isStationary" clearable placeholder="请选择是否有驻站人员">
          <el-option label="全部" value="_all"></el-option>
      ...
      <el-form-item label="协议签订">
        <el-select v-model="param.isAgreement" clearable placeholder="请选择是否协议签订">
          <el-option label="全部" value="_all"></el-option>
    ...
    <template #operate="scoped">
      <el-button type="primary"  @click="gotoPage('map', scoped.row)"> 地图 </el-button>

      <el-dropdown v-if="permission.singleUpdate" @command="(c) => handleCommend(c, scoped.row)">
        <el-button type="warning"  class="ml-10px">
          <DAliIcon name="dots-horizontal" color="white"></DAliIcon>
        </el-button>
        <template #dropdown>
          <el-dropdown-menu>
      ...
      <el-button v-else type="warning"  @click="handleShowVisit(scoped.row)">
        新增拜访
      </el-button>
    </template>
    <template #railwayStationName="scoped">
      <el-link type="primary" href="javascript:void(0);" @click="gotoPage('details', scoped.row)">
        {{ scoped.row.railwayStationName }}
      </el-link>
      <span class="theme-danger">
        <el-tooltip content="站点可用度" effect="light" placement="bottom">
          <span class="theme-danger cu-pointer"> ({{ (scoped.row.rate || 0).toFixed(2) }}%) </span>
        </el-tooltip>
      </span>
      <span v-if="scoped.row.stationLevelMark" class="fr">
        <el-tooltip :content="scoped.row.stationLevelName" effect="light" placement="bottom">
          <span
            class="cu-pointer"
            :style="{
      ...
      <div v-if="scoped.row.cmsNum || scoped.row.pictureNum">
        ...
        >
          <span class="theme-color cu-pointer" @click="handleShowCms(scoped.row)">
    ...
    <template #hasPricePolicy="scoped">
      <div class="station-tags">
        <el-tag :type="scoped.row.hasPricePolicy ? 'success' : 'info'" class="station-tag define">
    ...
    <template #hasPrivateLine="scoped">
      <div class="station-tags">
        <el-tag :type="scoped.row.hasPrivateLine ? 'success' : 'info'" class="station-tag define">
      ...
      <el-form-item label="">
        <el-dropdown @command="handleCommendAll" v-if="permission.allUpdate">
          <el-button type="danger">
            站点更新
            <DAliIcon name="arrow-down" class="ml-5px" />
          </el-button>
          <template #dropdown>
            <el-dropdown-menu>
        ...
        <el-button
          ...
          >新增</el-button
        >
        <el-button type="warning" class="ml-10px" @click="addBusiness(true)">
          等级更新
        </el-button>
      </el-form-item>

      <div class="fr" style="margin-right: 50px; line-height: 33px; cursor: pointer">
        <span class="theme-warning fs-14 visited" @click="showVisitCount(true)"> 拜访统计 </span>
      </div>
    </template>
    <template #querybutton> </template>
    <VisitRecordForm
      v-model:show="showVisit"
      :station-id="currentStation.id"
      :station-name="currentStation.railwayStationName"
    ></VisitRecordForm>
    <CmsDialog v-model="showCms" :show-query="true" :business-id="cmsId"></CmsDialog>
    <CmsDtDialog></CmsDtDialog>
    <VisitedCount v-model:show-dialog="showVisiteds" @close="showVisitCount(false)" />
    <EditForm v-model="showEdit" @success="handleSuccess"></EditForm>
    <import-excel
...
<script lang="ts" setup>
  ...
  import { tagColorMap, stationLevelColorMap, stationLevelTextMap } from './store'
  ...
  import { Download, PATH_URL } from '@/request'
  ...
  onMounted(() => {
    checkFunPermissionAsync(route.path, 'newRailwayStation').then((res) => {
