...
export function useList() {
  ...
  function renderShow(val: string, type: "BureauCode" | "TrainCode") {
    const options =
      type === "BureauCode" ? bureauOptions.value : trainDeptOptions.value;
    return options.find((x: any) => x.value === val || x.realValue === val)
      ?.text;
  }
  ...
  function loadData() {
    ...
    try {
      loading.value = true;
      const temp = deepClone<RailwayStationParam>(param.value);
  ...
  function gotoPage(type: "map" | "details", item: RailwayStationDto) {
    ...
    railwayStationStore.push(deepClone<RailwayStationDto>(item));
    railwayStationStore.setCurrent(deepClone<RailwayStationDto>(item));
    router.push({
      path:
        type === "map"
          ? "/resource-app/station-map"
          : "/resource-app/station-dt",
      query
    });
  }
  ...
  const TableColumn: DCTableColumn[] = [
...

    {
      ...
      formatter: (row: RailwayStationDto) => {
        ...
        const cityName = zxs.includes(row.provinceName)
          ? row.districtName
          : row.cityName;
