...
import { nextTick, render, h } from 'vue'
...
export function createPrivatePhone(phone: string, extraStr?: string, noClick = false) {
  ...
  const copyWrapEl = document.createElement('i')

  render(h(DAliIcon, { name: 'copy', class: 'inline-block ml-4px cursor-pointer' }), copyWrapEl)
  ...
  if (phone) {
    div.innerHTML =
      ...
      `<span class='phone' style='user-select:none'>${window.getPrivatePhone(phone)}</span>`
    // div.children[div.children.length - 1].insertAdjacentHTML(
    //   "afterend",
    //   ' <span class="copy-icon cursor-pointer" aria-label="复制">⧉</span>'
    // );
    div.innerHTML += copyWrapEl.innerHTML
    ...
    nextTick(() => {
      ...
      const phoneEle = Array.from(document.querySelector(`#${id}`)?.children || []).find(
        (e) => e.className.indexOf('phone') > -1
      )
